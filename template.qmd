...
#variable name
f2   <-  mass~bill_d+bill_l+species+sex+flip_l
...
```
```{r}
f1 <- mass~bill_d+bill_l+flip_l+species+sex+island
...
Split dta into test and training 
random sampling
what metric?
...
For loop 
  ...
  - make predictions on test set w fitted training model
Close loop
...
```{r}
training_lm1 <- lm(md1, data=train_df)
summary(training_lm1)

training_lm2 <- lm(md2, data=train_df)
summary(training_lm2)

training_lm3 <- lm(md3, data=train_df)
...
# w/o it creates larger vectors w/ each iteration
...
for( i in 1:folds){ 
  ...
  # Run for one model
  model_1 = lm(md1, data=kfold_train_df)
  
  #Get the predictions from the model
  kfold_predict <- kfold_test_df |>
    mutate(model = predict(model_1, kfold_test_df))
           
  # Summarize/calculate the rmse for that model
  kfold_rmse <- kfold_predict |>
    summarize(rmse_md1= calc_rmse(mass, model))
...
kfold_cv<-function(i,df,formula){
  ...
  kfold_rmse <- kfold_pred_df |>
    summarize(rmse_md1= calc_rmse(mass, md))
  
  return(kfold_rmse$rmse_md1)
